    ...
    fetch(sheetUrl)
        ...
        .then(csvText => {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                zoomCall.live = getDayAndTimeText(zoomCall.schedule[0].day, zoomCall.schedule[0].hour, zoomCall.schedule[0].minute);
...
// Function to convert day, hour, and minute to a formatted string
function getDayAndTimeText(day, hour, minute) {
    const daysOfWeek = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    const formattedHour = hour % 12 || 12; // Convert to 12-hour format
    const amPm = hour >= 12 ? "pm" : "am";
    const formattedMinute = minute.toString().padStart(2, "0");

    return `${daysOfWeek[day]} ${formattedHour}:${formattedMinute}${amPm} ET`;
}
...
function filterZoomCallsByEnrollment(calls, isHost) {
    const enrolledCourses = {{ contact.enrolled_course_ids | json }} || [];
    const enrolledSections = {{ contact.enrolled_section_ids | json }} || [];
...
function updateLiveRoomButton(zoomCalls) {
    ...
    if (nextCall) {
        ...
        if (isHost) {
            // Hosts see all upcoming meetings
            if (currentDay === schedule.day && currentMinutesSinceMidnight >= soonStartMinutesSinceMidnight && currentMinutesSinceMidnight < startMinutesSinceMidnight) {
            ...
            // Attendees see only the "Starting Soon" or "Room Open" status within relevant time frames
            if (currentDay === schedule.day && currentMinutesSinceMidnight >= soonStartMinutesSinceMidnight && currentMinutesSinceMidnight < startMinutesSinceMidnight) {
                newText = `${nextCall.CallName} - ${nextCall.live} - STARTING SOON`;
                newHref = targetUrl;
            } else if (currentDay === schedule.day && currentMinutesSinceMidnight < soonStartMinutesSinceMidnight) {
                // For attendees, do not show meetings too far in the future
                newText = "Live Room Is Currently Offline";
            }
        }
        newHref = targetUrl;
    }

...
// Function to build live time string from day, hour, and minute
function buildLiveTimeString(schedule) {
    const dayName = getDayName(schedule.day);
    const timeString = formatTime(schedule.hour, schedule.minute);
    return `${dayName} ${timeString} ET`;
}
    ...
    fetch(sheetUrl)
        ...
        .then(csvText => {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                // Build live time string
                zoomCall.live = buildLiveTimeString(zoomCall.schedule[0]);

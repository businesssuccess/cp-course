...
function filterZoomCallsByEnrollment(calls, isHost) {
    const enrolledCourses = []; // Replace with page context or data injection if necessary
    const enrolledSections = []; // Replace with page context or data injection if necessary
...
function isUserHost(call, email) {
    return (
        call.hosts1 === email ||
        call.hosts2 === email ||
        call.hosts3 === email ||
...
function updateLiveRoomButton(zoomCalls, loggedInUserEmail) {
    ...
    const isHost = zoomCalls.some(call => isUserHost(call, loggedInUserEmail));
    ...
    if (nextCall) {
        ...
        const targetUrl = isUserHost(nextCall, loggedInUserEmail) && nextCall.hostLink ? nextCall.hostLink : nextCall.ZoomOrObvioURL;
        const prefixText = isUserHost(nextCall, loggedInUserEmail) ? "HOST: " : "";
...
document.addEventListener('DOMContentLoaded', function() {
    const email = "{{ contact.email }}"; // Pass the email as an argument to avoid global declaration
    fetchZoomCallsFromSheet(zoomCalls => updateLiveRoomButton(zoomCalls, email));
    setInterval(() => fetchZoomCallsFromSheet(zoomCalls => updateLiveRoomButton(zoomCalls, email)), 30000);
});
